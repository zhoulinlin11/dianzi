...
import com.example.asus.caojiyang20180521.ui.WebActivity;
import com.example.asus.caojiyang20180521.ui.adapter.XrvAdapter;
import com.example.asus.caojiyang20180521.ui.adapter.XrvAdapter2;
...
public class Fragment2  extends BaseFragment<SatinPresenter> implements SatinContract.View {
    private XRecyclerView xrv;
    private XrvAdapter2 xrvAdapter2;
    ...
    public void initView(View view) {
        xrv = view.findViewById(R.id.xrv2);
        mPresenter.getSatinBean(4,1);
    }

    @Override
    public void getSatinBeanSuccess(SatinBean satinBean) {
        final List<SatinBean.DataBean> data = satinBean.getData();
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getContext());
        xrv.setLayoutManager(linearLayoutManager);
        //添加Android自带的分割线
        xrv.addItemDecoration(new DividerItemDecoration(getContext(), DividerItemDecoration.VERTICAL));
        //设置适配器
        xrvAdapter2 = new XrvAdapter2(getContext(), data);
        xrv.setAdapter(xrvAdapter2);
        //点击事件
        xrvAdapter2.setOnItemClickListener(new XrvAdapter2.OnItemClickListener() {
            @Override
            public void onItemClick(int position) {
                String videouri = data.get(position).getVideouri();
                Intent intent = new Intent(getContext(), WebActivity.class);
                intent.putExtra("vvv",videouri);
